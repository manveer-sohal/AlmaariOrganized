...
function ViewOutfits() {
  ...
  return (
    <div className="p-4 w-full max-w-2xl mx-auto">
      <div className="absolute top-1/2 right-1/3 z-10 bg-white/80 backdrop-blur border border-indigo-200 rounded-xl p-3 shadow-md">
        {activeId && <Delete uniqueId={activeId} />}
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              {outfits.map((o) => (
                <button
                  key={o.uniqueId}
                  className={`w-full text-left px-3 py-2 rounded-lg border grid grid-cols-2 items-center ${
                    o.uniqueId === activeId
                ...
                >
                  ...
                  <div
                    className="text-xs opacity-80 col-span-2 justify-self-end"
                    onClick={() => handleDelete(o.uniqueId)}
                  >
                    delete
                  </div>
                </button>
